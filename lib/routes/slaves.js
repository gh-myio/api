...
async function findAllGHome (req, res, next) {
  const token = req.headers['x-access-token']
  const allowedAmbients = await utils.getUserAllowedAmbients(token)
...
async function create (req, res, next) {
  ...
  try {
    ...
    setTimeout(() => require('../WebSocketHandler').send(JSON.stringify({
      type: 'admin',
      command: 'start_slave',
      id: slave.id
    })), 1000)
