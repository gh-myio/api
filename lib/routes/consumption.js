...
async function queryYearlyData (start, end) {
  const data = await sequelize.query(`
      WITH per_slave AS (
...
async function queryMonthlyData (start, end) {
  const data = await sequelize.query(`
      WITH per_slave AS (
               SELECT time_bucket('1 hour', timestamp) AS time,
               (SUM(value) / COUNT(value)) AS value,
...
async function queryDataByType (type, start, end) {
  let bucket
  switch (type) {
    case 'minute':
      bucket = '5 minutes'
      return queryAllData(bucket, start, end)
    case 'hour':
      bucket = '1 hour'
      return queryAllData(bucket, start, end)
    case 'day':
      return queryMonthlyData(start, end)
    case 'month':
      return queryYearlyData(start, end)
  }
}
